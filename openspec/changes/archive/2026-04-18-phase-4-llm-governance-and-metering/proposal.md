...
- Add a contract-level backend governance slice that implements provider routing, shared health state, budget reservation and settlement, model-catalog validation, and metering rollups without introducing live provider SDK or Redis integration yet.
...
- Backend verification now includes contract tests for provider failover, atomic reservations, rollup reproducibility, and reconciliation drift behavior before the change can be archived.
